...
$stmt = $db->prepare('SELECT id, topic_id, user_id, body, created_at FROM replies WHERE id = :id');
...
if (!can_edit_owned($user, (int)$reply['user_id'], $reply['created_at'], $editWindowMinutes)) {
    http_response_code(403);
    echo 'Вы не можете редактировать этот ответ.';
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($body === '') {
        ...
        log_action('reply_edit', (int)$user['id'], $user['username'], 'reply', $id, array(
